...
public class Item implements Comparable, Serializable {
    ...
    public Item() {
        ...
        System.out.println(Level.possibleItems == null);
        int pickVals = rand.nextInt(Level.possibleItems.length);// random index
                                                                // from
                                                                // possibleNames,
                                                                // values and
                                                                // weights lists
     ...
     * 
     * @param ratio
     *            Differzenz zwischen Ankaufspreis und Verkaufspreis für den
     *            Spieler
     * 
     */
     ...
     * 
     * @param name
     *            des Händler TraderJoe oder TraderJeb
     * @param ratio
    ...
    public Item(String name, int ratio) {
        this.Namen = name;
        int i = 0;
        while (i < Level.possibleItems.length
                && !(Level.possibleItems[i]).getName().equals(name)) {
            i++;
        }
    ...
    public boolean equals(Object obj) {
        ...
        if (compare.Namen.toUpperCase() == this.Namen.toUpperCase()
                && compare.Wert == this.Wert && compare.Gewicht == this.Gewicht) {
            return true;
        } else {
            return false;
        }
    ...
    public String toString() {
        return " Name: " + this.Namen + ", Wert: " + this.Wert + ", Gewicht: "
                + this.Gewicht;
    }

    public int compareTo(Object o) throws IllegalArgumentException { // returns
                                                                     // -1 if
                                                                     // current
                                                                     // item
                                                                     // alphabetically
                                                                     ...
                                                                     // proceeds
                                                                     // object
        if (!(o instanceof Item) || o == null) { // returns +1 if follows, 0 if
                                                 // all values are identical (if
                                                 // 0, increase
                                                 // item count of this in
                                                 // inventory by #of object)
            throw new IllegalArgumentException(
                    "comparing object does not exist or is not an item");
        }

        Item compare = (Item) o;
        if (this.Namen.toUpperCase().compareTo(compare.Namen.toUpperCase()) != 0) {
            return this.Namen.toUpperCase().compareTo(
                    compare.Namen.toUpperCase());
