public class LinkedList <T extends Comparable> implements List<T>{
    ...
    public boolean isInList(T x){ //returns whether item is in list or not
        ...
        while(current.getNext() != null && inList==false){
            ...
            if(current.getNext().getItem() instanceof Quest){ //returns true if quest both exists and is complete
                if((current.getNext().getItem()).getName().equals(((Quest)temp.getItem()).getPreReqs())){
    ...
    public int indexInList(String name){//returns index of item in list
        ...
        while(current.getNext() != null && !inList){
            if(current.getNext().getItem().getName().toUpperCase().equals(name.toUpperCase())){
                inList=true;
                break;
            }
            i++;
    ...
    public List<T> insert(T x){ //inserts item in order of sort (use to automatically sort items as they're added)
        Node temp = new Node(x);
        Node current = this.firstItem;
        while(current.getNext() != null && current.getNext().getItem().compareTo(temp.getItem())<0){
            current=current.getNext();
        }
        if(current.getNext() != null && current.getNext().getItem().compareTo(temp.getItem())==0){ //are the same
